...
class AttackPlanningAgent:
    ...
    #     ✅ 根据初始化参数决定是否包含trace请求
    #     """
    #     return self.crawler._generate_execution_graph_with_requests(
    ...
    def _load_attack_graph(self) -> Dict:
        ...
        filtered_graph = self._filter_graph_requests(full_graph, max_requests=80) # 注意这里是80，你日志里好像是20？
        
        self._cached_graph = filtered_graph # 缓存起来
        return filtered_graph
    
    # def _identify_request(self, request_dict: Dict) -> Tuple:
    #     """
    #     将图中的请求字典转换为统一的dedup_key
        
    #     支持所有5种请求类型：
    #     - page_requests: 页面加载时的请求
    #     - tasks[].requests: 任务执行时的请求
    #     - outgoing_links: 外链和动态跳转
    #     - trace_requests: 从trace文件提取的孤立请求
    #     - pre_navigation_requests: 预跳转请求（如登录）
        
    #     Args:
    #         request_dict: 请求字典，至少包含 method 和 url
        
    #     Returns:
    #         dedup_key: 可用于唯一标识请求的元组
    #     """
    #     method = request_dict.get("method", "GET")
    #     url = request_dict.get("url", "")

    #     # 处理POST/PUT/PATCH请求的body_schema
    #     body_schema = request_dict.get("body_schema")
    #     if body_schema and method.upper() in ["POST", "PUT", "PATCH"]:
    #         # 从body_schema构造假body（与_get_or_create_request保持一致）
    #         keys = [k.strip() for k in body_schema.split(',')]
    #         fake_body = json.dumps({k: "" for k in keys}, sort_keys=True)
    #     else:
    #         fake_body = None

    #     # 使用crawler的去重函数生成标准key
    #     return self.crawler._make_dedup_key(method, url, fake_body)
    ...
    # def _is_request_in_current_graph(self, method: str, url: str) -> bool:
    #     """
    #     检查请求是否在当前图中
    ...
    #     """
    #     # 先检查request_mapping（page_requests和task requests）
    #     key = (method, url)
    ...
    #     示例输入：
    #     Action: analyze_requests
        
    #     Selected_Requests:
    #     GET http://example.com/api/user/123
    #     POST http://example.com/api/user/123/update
        
    #     Reasoning:
    #     These requests involve user resource access...
        
    #     Attack_Tasks:
    #     [TASK001] SQL_INJECTION | admin | Test whether the id parameter in user query is vulnerable to SQL injection
    #     [TASK002] XSS | user1 | Check if the username field in update request reflects unescaped input
    #     [TASK003] IDOR | user1 -> user2 | Test if user1 can access user2's data by changing the id parameter
    #     """
    #     try:
    ...
    #         if not action_match:
    #             action_type = ""  # 没有Action字段，给空字符串
    #             self._log("[AttackPlanningAgent] No Action field found, will try to detect intent")
    #         else:
    #             action_type = action_match.group(1).strip().lower()
    #             self._log(f"[AttackPlanningAgent] Detected action: {action_type}")

    #         # ✅ 修复：用子串匹配代替严格相等，更灵活
    #         # 如果有 "analyze" + "request"，或者有 Attack_Tasks，就进入 analyze 分支
    #         has_attack_tasks = bool(re.search(r'attack[_\s]*task', response, re.IGNORECASE))

    #         if ("analyze" in action_type and "request" in action_type) or (not action_type and has_attack_tasks):
    #             # 提取 Selected_Requests
    #             # requests_match = re.search(
    #             #     r'Selected_Requests:\s*\n(.*?)\n\s*(?:Note:|Reasoning:)',
    #             #     response,
    #             #     re.DOTALL
    #             # )
    #             # 更宽松的正则，匹配任何包含 reasoning 的行
    #             requests_match = re.search(
    #                 r'Selected_Requests:\s*\n(.*?)\n\s*(?:\*\*)?(?:Note|Reasoning)',
    #                 response,
    ...
    #                 self._log("[AttackPlanningAgent] No Selected_Requests found")
    #                 return (None, None)
    ...
    #                 # ✅ 修复：解析格式 "METHOD URL {BODY_SCHEMA}?"
    #                 # 例如：POST http://url {name,age} 或 GET http://url
    #                 match = re.match(
    ...
    #                     url = match.group(2)
    #                     body_schema = match.group(3)  # 可能为None

    #                     if body_schema:
    #                         selected_requests.append((method, url, body_schema.strip()))
    #                     else:
    #                         selected_requests.append((method, url, None))
    #                 else:
    #                     # 兼容旧格式（没有body_schema）
    #                     parts = line.split(None, 1)
    #                     if len(parts) == 2:
    #                         selected_requests.append((parts[0], parts[1], None))
    ...
    #             # ✅ 修复：用子串匹配找 Attack Tasks，然后逐行提取任务
    #             lines = response.split('\n')
    #             attack_tasks_start_idx = -1

    #             # 找到包含 "attack" 和 "task" 的行（忽略格式：星号、下划线、大小写等）
    #             for i, line in enumerate(lines):
    #                 line_cleaned = line.lower().replace('*', '').replace('_', ' ').strip()
    #                 if 'attack' in line_cleaned and 'task' in line_cleaned:
    #                     attack_tasks_start_idx = i
    #                     self._log(f"[Parse] Found Attack Tasks section at line {i}: {line.strip()}")
    #                     break

    #             if attack_tasks_start_idx == -1:
    #                 self._log("[AttackPlanningAgent] ⚠️ No Attack Tasks section found")
    #                 return (None, None)

    #             # 从找到的位置开始，逐行提取任务（不会被空行或错误行打断）
    #             task_lines = []
    #             for i in range(attack_tasks_start_idx + 1, len(lines)):
    #                 line = lines[i].strip()
    ...
    #                     self._log(f"[Parse] ✓ Extracted: {parsed.get('task_id')}")
    #                 else:
    #                     self._log(f"[Parse] ✗ Failed to parse: {line[:100]}...")

    #             if not task_lines:
    #                 self._log("[AttackPlanningAgent] ⚠️ No valid tasks extracted")
    #                 return (None, None)

    #             self._log(f"[AttackPlanningAgent] Extracted {len(task_lines)} tasks")
    ...
    #         elif "skip" in action_type and "request" in action_type:
    #             # ========== 新增：处理skip_requests ==========
    #             # 提取 Skipped_Requests
    #             requests_match = re.search(
    ...
    #             )
    #             if not requests_match:
    #                 self._log("[AttackPlanningAgent] No Skipped_Requests found")
    #                 return (None, None)
    ...
    #                 # ✅ 修复：解析格式 "METHOD URL {BODY_SCHEMA}?"
    #                 match = re.match(r'(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+(http\S+?)(?:\s+\{([^}]+)\})?', line, re.IGNORECASE)
    #                 if match:
    #                     method = match.group(1)
    #                     url = match.group(2)
    #                     body_schema = match.group(3)  # 可能为None
    ...
    #                     if body_schema:
    #                         skipped_requests.append((method, url, body_schema.strip()))
    #                     else:
    #                         skipped_requests.append((method, url, None))
    #                 else:
    #                     # 兼容旧格式
    #                     parts = line.split(None, 1)
    #                     if len(parts) == 2:
    #                         skipped_requests.append((parts[0], parts[1], None))
    ...
    #             self._log(f"[AttackPlanningAgent] Extracted {len(skipped_requests)} requests to skip")
    ...
    #     支持两种格式：
    #     - GET: [TASK001] SQL_INJECTION | GET http://url | Test description
    #     - POST (with {}): [TASK001] SQL_INJECTION | POST http://url {body_schema} | Test description
    #     - POST (without {}): [TASK001] SQL_INJECTION | POST http://url body_schema | Test description
    ...
    #             "body_schema": "age,name",  # 仅POST/PUT/PATCH有此字段
    #             "description": "Test description"
    #         }
    #     """
    #     # ✅ 修复：允许 body_schema 不带 {}
    #     # 格式：[TASKXXX] VULN_TYPE | METHOD URL_AND_MAYBE_SCHEMA | DESCRIPTION
    #     match = re.match(
    #         r'\[(\w+)\]\s+([A-Z_]+)\s+\|\s+(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+(.*?)\s+\|\s+(.+)',
    #         task_line.strip(),
    #         re.IGNORECASE
    #     )

    #     if not match:
    #         self._log(f"[Warning] Invalid task format: {task_line}")
    #         # 尝试兼容旧格式（带账户的）
    #         return self._parse_task_line_legacy(task_line)

    #     task_id, vuln_type, method, middle_part, description = match.groups()

    #     # 从 middle_part 提取 URL 和可选的 body_schema
    #     # middle_part 可能是：
    #     # - "http://url" (GET 或没有 body_schema 的 POST)
    #     # - "http://url {body_schema}" (POST with {})
    #     # - "http://url body_schema" (POST without {})
    #     parts = middle_part.split(None, 1)  # 最多分割成2部分（以空格分割）
    #     url = parts[0].strip()
    #     body_schema = None
    ...
    #         # 移除可能的 {} 并清理
    #         body_schema = schema_part.strip('{}').strip()
    ...
    def _parse_task_line(self, task_line: str) -> Optional[Dict]:
        ...
        # 🆕 新正则
        match = re.match(
            r'\[(\w+)\]\s+([A-Z_]+)\s+\|\s+REF_ID:\s*([a-fA-F0-9]+)\s+\|\s+(.+)',
            task_line.strip(),
            re.IGNORECASE
        )
    ...
    #     """
    #     执行分析动作 - 生成AttackTask对象列表

    #     Returns:
    #         List[AttackTask]
    #     """
    #     selected_requests = action_data.get("selected_requests", [])
    #     reasoning = action_data.get("reasoning", "")
    #     task_lines = action_data.get("task_lines", [])

    #     self._log(f"\n[AttackPlanningAgent] Analyzing {len(selected_requests)} requests")
    #     self._log(f"[AttackPlanningAgent] Generated {len(task_lines)} tasks")
    #     if reasoning:
    #         self._log(f"[AttackPlanningAgent] Reasoning: {reasoning}")

    #     # ========== 关键修复：验证请求是否在当前图中 ==========
    #     valid_selected_requests = []
    #     for req_tuple in selected_requests:
    #         # ✅ 修复：解析三元组 (method, url, body_schema)
    #         if len(req_tuple) == 3:
    #             method, url, body_schema = req_tuple
    #         else:
    #             # 兼容旧格式 (method, url)
    #             method, url = req_tuple[0], req_tuple[1]
    #             body_schema = None

    #         # ✅ 使用body_schema构造dedup_key
    #         # 注意：这里需要从body_schema反推body，但我们没有完整body
    #         # 解决方案：直接用body_schema构造key
    #         req_dict = {"method": method, "url": url}
    #         if body_schema:
    #             req_dict["body_schema"] = body_schema

    #         key = self._identify_request(req_dict)

    #         # 检查是否已经被处理
    #         if key in self.state.tested_requests:
    #             self._log(f"[Warning] Request already tested, skipping: {method} {url} {{{body_schema or ''}}}")
    #             continue

    #         # 检查是否在当前图中
    #         # 需要特殊处理：图中的请求可能没有body_schema信息
    #         # 所以我们需要用(method, url)来检查是否存在，然后用完整key来去重
    #         # in_graph = self._is_request_in_current_graph(method, url)
    #         # 检查是否在当前图中
    #         in_graph = self._is_request_in_current_graph(method, url, body_schema)

    #         if not in_graph:
    #             self._log(f"[Warning] Request not in current graph, skipping: {method} {url}")
    #             continue

    #         valid_selected_requests.append((method, url, body_schema))

    #     self._log(f"[AttackPlanningAgent] Valid requests after filtering: {len(valid_selected_requests)}/{len(selected_requests)}")

    #     # 如果没有有效请求，直接返回
    #     if not valid_selected_requests:
    #         self._log("[AttackPlanningAgent] No valid requests to process")
    #         return []

    #     # ✅ 修复：先解析任务行并构造AttackTask对象，再标记请求为已处理
    #     # （避免标记后导致 _load_attack_graph 时请求被过滤掉）
    #     attack_tasks = []
    ...
    #             self._log(f"[Error] Task {parsed['task_id']}: Invalid REF_ID {ref_id}, not found in map")
    #             continue
    ...
    #         url = parsed.get("url")
    #         body_schema = parsed.get("body_schema")  # ✅ 获取body_schema
    #         description = parsed["description"]

    #         # # ========== 新格式：直接使用method、url和body_schema ==========
    #         # if method and url:
    #         #     # 新格式：Task包含明确的METHOD、URL和可选的BODY_SCHEMA
    #         #     # ✅ 使用body_schema来查找完整请求
    #         #     target_request = self._get_or_create_request(method, url, body_schema)

    #         #     if not target_request:
    #         #         self._log(f"[Warning] Task {task_id}: Cannot find or create request for {method} {url} {{{body_schema or ''}}}")
    #         #         continue

    #         # else:
    #         #     # 旧格式兼容：通过描述模糊匹配
    #         #     self._log(f"[Warning] Task {task_id} uses legacy format (no METHOD URL), falling back to fuzzy matching")
    #         #     target_request = self._find_matching_request(
    #         #         valid_selected_requests,
    #         #         description
    #         #     )

    #         #     if not target_request:
    #         #         self._log(f"[Warning] Task {task_id}: Cannot match request from description")
    #         #         continue
    ...
    #         self._log(f"  [{task_id}] {vuln_type} | {method} {url} | {account}")

    #         # ✅ 记录到planned_attacks列表（用于JSON导出）
    #         # ✅ 保存完整请求对象，包括headers、body、response等
    ...
    #                 # 响应体可能很大，截断前1000字符避免文件过大
    #                 "response_body": (target_request.get("response_body", "") or "")[:1000] if target_request.get("response_body") else None
    #             }
    #         })
    ...
    #     # # ✅ 修复：所有任务解析完成后，再标记请求为已处理
    #     # for req_tuple in valid_selected_requests:
    #     #     # ✅ 解析三元组 (method, url, body_schema)
    #     #     method, url, body_schema = req_tuple

    #     #     # ✅ 使用body_schema来获取完整请求
    #     #     target_request = self._get_or_create_request(method, url, body_schema)

    #     #     if target_request:
    #     #         body = target_request.get("body")
    #     #     else:
    #     #         # 如果无法获取完整请求，从body_schema构造假body
    #     #         if body_schema and method.upper() in ["POST", "PUT", "PATCH"]:
    #     #             keys = [k.strip() for k in body_schema.split(',')]
    #     #             body = json.dumps({k: "" for k in keys}, sort_keys=True)
    #     #         else:
    ...
    #     #     # 统一用 LLM/图视角的 schema 来生成 key（和 pending / filter / compare 完全一致）
    #     #     req_dict = {"method": method, "url": url}
    #     #     if body_schema:
    #     #         req_dict["body_schema"] = body_schema
    #     #     key = self._identify_request(req_dict)

    #     #     # processed_requests 也用同一个 key（避免 body 解析差异导致不一致）
    ...
    def _execute_analyze_action(self, action_data: Dict) -> List[AttackTask]:
        """
        执行分析动作 (ID 化版本)
        直接使用 Task 行中的 REF_ID 查找原始请求，不再依赖 URL/Body 匹配。
        """
        task_lines = action_data.get("task_lines", [])
        reasoning = action_data.get("reasoning", "")
        ...
        for task_line in task_lines:
            # 解析任务行 (需确保 _parse_task_line 已更新为支持 REF_ID)
            parsed = self._parse_task_line(task_line)
            if not parsed: 
                continue
            
            task_id = parsed["task_id"]
            ref_id = parsed["ref_id"]  # 👈 核心：获取 ID
            description = parsed["description"]
            vuln_type = parsed["vuln_type"]
            ...
            if ref_id in self.state.tested_requests:
                self._log(f"[Warning] Request {ref_id} already tested, skipping task {task_id}")
                continue

            # 2. 核心：通过 ID 直接获取原始请求数据
            # request_id_map 是在 crawler._build_request_mapping 中构建的
            target_request = self.crawler.request_id_map.get(ref_id)
            
            if not target_request:
                self._log(f"[Error] Task {task_id}: Invalid REF_ID '{ref_id}' - not found in crawler map")
                continue
            
            # 3. 生成 AttackTask 对象
            # 这里的 target_request 是最原始的数据，包含完整的 Raw Body
            account = self.default_account if self.default_account else "default_account"
            ...
            self.planned_attacks.append({
                ...
                "target_request": {
                    ...
                    "headers": target_request.get("headers", {}),
                    "query_params": target_request.get("query_params", {}),
                    "body": target_request.get("body"), # 原始 Body
                    # 响应体可能很大，截断
            ...
            # 同步更新 crawler 的 processed 集合，确保图生成时能正确过滤
            # 注意：如果 crawler.processed_requests 存的是 tuple key，这里可能需要适配
            # 建议将 crawler.processed_requests 也改为存 ID，或者这里根据 target_request 反推 Key
            # 简单方案：只在 Agent 侧维护 tested_requests 用于过滤 Prompt
            # 但为了保持一致性：
            if hasattr(self.crawler, 'processed_requests'):
                # 尝试用 ID 作为 Key 存入 (如果 crawler 支持) 
                # 或者反向生成去重 Key
    ...
    #     """
    #     执行skip动作 - 标记请求为已处理但不生成任务

    #     Args:
    #         action_data: 包含skipped_requests和reasoning

    #     Returns:
    #         跳过的请求数量
    #     """
    #     skipped_requests = action_data.get("skipped_requests", [])
    #     reasoning = action_data.get("reasoning", "")

    #     self._log(f"\n[AttackPlanningAgent] Skipping {len(skipped_requests)} untestable requests")
    #     if reasoning:
    #         self._log(f"[AttackPlanningAgent] Reasoning: {reasoning}")

    #     # 验证并标记
    #     valid_count = 0
    #     for req_tuple in skipped_requests:
    #         # ✅ 解析三元组 (method, url, body_schema)
    #         if len(req_tuple) == 3:
    #             method, url, body_schema = req_tuple
    #         else:
    ...
    #         # 检查是否已经被处理
    #         if key in self.state.tested_requests:
    #             self._log(f"[Warning] Request already processed, skipping: {method} {url} {{{body_schema or ''}}}")
    #             continue
    ...
    #         if not in_graph:
    #             self._log(f"[Warning] Request not in current graph, cannot skip: {method} {url}")
    #             continue
    ...
    #         self._log(f"  [SKIPPED] {method} {url} {{{body_schema or ''}}}")

    #     self._log(f"[AttackPlanningAgent] Successfully skipped {valid_count}/{len(skipped_requests)} requests")
