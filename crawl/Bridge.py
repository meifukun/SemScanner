...
class Bridge:
    ...
    def _open_task_log(self, photo_dir, task_id: str):
        ...
        self._log(f"[Bridge] Task log created: {self._task_log_path}")

    def _close_task_log(self):
        """Close task log file"""
        if self._task_log_file:
            self._log(f"[Bridge] Task log closed")
    ...
    def query_llm(self, prompt: str) -> str:
        try:
            ...
            self._log(f"[Bridge] LLM call failed: {e}")
            return "STOP"
    ...
    def execute_task_step(self, task: Task, action_id: int, value: Optional[str] = None, force_event: bool = False) -> bool:
        ...
        try:
            ...
            # ===== DEBUG: Check mapping state =====
            self._log(f"[Bridge] [DEBUG] Checking mapping state before execution:")
            self._log(f"  - action_id: {action_id}")
            self._log(f"  - force_event: {force_event}")

            # Check actions_mapping
            actions_mapping = self.sensors.get_actions_mapping()
            self._log(f"  - actions_mapping size: {len(actions_mapping.mapping)}")
            self._log(f"  - actions_mapping keys (first 20): {list(sorted(actions_mapping.mapping.keys()))[:20]}")

            # If the ID to execute is in actions_mapping, show detailed info
            if action_id in actions_mapping.mapping:
                mapping_info = actions_mapping.mapping[action_id]
                locator = mapping_info.get('locator', mapping_info.get('primary', 'N/A'))
                elem_type = mapping_info.get('type', 'N/A')
                self._log(f"  - actions_mapping[{action_id}]: type={elem_type}, locator={locator[:80]}")
            else:
                self._log(f"  - actions_mapping[{action_id}]: NOT FOUND")

            # Check event_mapping
            self._log(f"  - event_mapping size: {len(event_mapping.mapping)}")
            self._log(f"  - event_mapping keys (first 10): {list(event_mapping.mapping.keys())[:10]}")
            self._log(f"  - has_event({action_id}): {event_mapping.has_event(action_id)}")

            # TRIGGER command: only execute event, no fallback
            if force_event:
                if event_mapping.has_event(action_id):
                    self._log(f"[Bridge] [DEBUG] TRIGGER command: found event ID {action_id}, starting execution")
                    return self._execute_event(action_id, value)
                else:
                    self._log(f"[Bridge] TRIGGER failed: event ID {action_id} does not exist")
                    self._log(f"[Bridge] [DEBUG] Available event ID list: {list(event_mapping.mapping.keys())}")
                    return False

            # Normal command: prioritize event, fallback to element
            if event_mapping.has_event(action_id):
                # This is an event ID, use event executor
                self._log(f"[Bridge] [DEBUG] Found event ID {action_id}, using event executor")
                return self._execute_event(action_id, value)
            else:
                # This is an element ID, use the original Actuators
                self._log(f"[Bridge] [DEBUG] Event ID {action_id} not found, falling back to element executor")
                success = self.actuators.execute_action_by_id(action_id, value=value)
                return success
        except Exception as e:
            # Catch any exception, print error and return failure
            self._log(f"[Bridge] Error during operation execution: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _execute_event(self, event_id: int, value: Optional[str] = None) -> bool:
        ...
        if not event_info:
            self._log(f"[Bridge] Event ID {event_id} does not exist")
            return False
        ...
        self._log(f"[Bridge] Executing event {event_id}: type={event_type}, text='{text}', xpath={xpath[:80]}...")

        # Locate element
        try:
            elem = self.driver.find_element(By.XPATH, xpath)
        except Exception as e:
            self._log(f"[Bridge] XPath locating failed: {e}")
            ...
            if original_id:
                try:
                    ...
                    self._log(f"[Bridge] Fallback succeeded: using ID={original_id}")
                except:
                    pass
                ...
                try:
                    ...
                    for candidate in candidates:
                        if text in candidate.text:
                            elem = candidate
                            self._log(f"[Bridge] Fallback succeeded: using class+text")
                            break
                except:
                    pass

            if not elem:
                self._log(f"[Bridge] All locating methods failed")
                return False
        ...
        try:
            ...
            if success:
                self._log(f"[Bridge] Event execution succeeded")
            else:
                self._log(f"[Bridge] Event execution failed")
            return success
        except Exception as e:
            self._log(f"[Bridge] Event execution exception: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _check_wrapper_errors(self):
        """
        DEBUG: Check browser event capture information
        """
        try:
            # Read non-intrusively captured events
            captured_events = self.driver.execute_script("return window.captured_events || [];")

            if captured_events:
                event_count = len(captured_events)
                self._log(f"[Bridge] Captured event count: {event_count}")

                # Show the most recent 5 events
                if event_count > 0:
                    recent_events = captured_events[-5:]
                    self._log(f"[Bridge] Recent events:")
                    for evt in recent_events:
                        self._log(f"  [{evt.get('counter')}] {evt.get('event_type')} on {evt.get('tag')} (id={evt.get('id') or 'N/A'})")
            else:
                self._log(f"[Bridge] No events captured")

        except Exception as e:
            self._log(f"[Bridge] Event capture check failed: {e}")
    ...
    def run_task(self, task, photo_dir, logging_in=False):
        ...
        try:
            self._log("-------------------------------------------------")
            self._log(f"[Bridge] Starting task {task.task_id}: {task.description}")
            ...
            while not task.completed:
                current_url = self.driver.current_url
                # # Don't cache, update everything directly. Choose one of the two approaches below.
                self.sensors.update_abstract_page()

                # # Detect URL change, load new page's abstraction and mapping from Store
                # if current_url != last_url:
                #     self._log(f"[Bridge] Page change detected: {last_url} -> {current_url}")
                #     # Use global Store instance
                #     if not self._store:
                #         self._log(f"[Bridge] Error: Store instance not initialized!")
                #         task.set_completed()
                #         break
                #     page = self._store.get_page(current_url)
                #     if page and page.abstract_page:
                #         # Update sensors' page abstraction
                #         self.sensors.abstract_page = page.abstract_page
                #         self._log(f"[Bridge] Loaded new page abstraction from Store ({len(page.abstract_page)} chars)")

                #         # Clear and rebuild element mapping
                ...
                #                 )
                #             self._log(f"[Bridge] Restored {len(self.sensors.actions_mapping.mapping)} element mappings")
                #         else:
                #             self._log(f"[Bridge] Warning: no actions_mapping in new page cache")

                #         # Clear and rebuild event mapping
                ...
                #             self._log(f"[Bridge] Restored {len(self.sensors.event_mapping.mapping)} event mappings")
                #         else:
                #             self._log(f"[Bridge] Warning: no event_mapping in new page cache")
                #     else:
                #         # This shouldn't happen because new pages should already be cached
                #         self._log(f"[Bridge] Error: new page {current_url} not found in Store!")
                #         self._log(f"[Bridge] This may mean tracer's on_new_page callback didn't work properly, or currently in login task")
                #         self.sensors.update_abstract_page()
                #     last_url = current_url
                #     need_rescan = False  # URL change handled, reset flag
                # # If rescan is needed (DOM changed but URL didn't, e.g. CONTINUE case)
                # elif need_rescan:
                #     self._log(f"[Bridge] DOM may have changed (CONTINUE case), rescanning current page: {current_url}")
                #     self.sensors.update_abstract_page()

                #     # DEBUG: record scan log to task file
                #     if hasattr(self.sensors, 'debug_scan_log'):
                #         self._log("\n" + "\n".join(self.sensors.debug_scan_log))
                #     need_rescan = False  # Reset flag


                # DEBUG: record scan log to task file (after each scan)
                if hasattr(self.sensors, 'debug_scan_log'):
                    self._log("\n" + "\n".join(self.sensors.debug_scan_log))
                page_description = self.sensors.get_abstract_page()
                self._log(f"[Bridge] LLM input\n{page_description}")
                prompt = self.generate_bridge_onestep_prompt(task, page_description, task.get_history())
                command_text = self.query_llm(prompt)
                self._log(f"[Bridge] LLM output: {command_text}")
                ...
                if "CONTINUE Decision:" in command_text:
                    ...
                    if decision_start != -1 and command_start != -1:
                        ...
                        self._log(f"[Bridge] {decision_text}")
                # # Step 1: Split text by "Command:"
                try:
                    command_parts = command_text.split("Command:")
                    commands = command_parts[1].strip().splitlines()
                except Exception as e:
                    self._log(f"[Bridge] Failed to parse command ({e}). LLM output may not follow format {command_text}, retrying this step.")
                ...
                for command_text in commands:
                    ...
                    self._log(f"[Bridge] Parsed command: kind={kind}, id={action_id}, value={value}")
                    if not kind:
                        self._log(f"[Bridge] Unrecognized command: {raw_cmd}")
                        ...
                        self._log(f"[Bridge] Skipping unrecognized command, continuing with subsequent commands")
                        continue  # Skip this iteration, continue next command
                    elif kind in ("DONE"):
                        self._log(f"[Bridge] Received ending command: {kind}, ending current task")
                        ...
                        self._log(f"[Bridge] Received failure command: {kind}, ending current task")
                        # Record history in one-line format (occurs on old_url page)
                        task.clear_history()
                    ...
                    try:
                        ...
                        self._log(f"[Bridge] Failed to parse command ({e}). LLM output action may not match {command_text}, retrying this step.")
                        continue  # Go to next while iteration, re-query LLM
                    ...
                    if kind == "CLICK":
                        ...
                        # DEBUG: check browser wrapper errors after CLICK
                        self._check_wrapper_errors()
                    elif kind == "TRIGGER":
                    ...
                    # PRESS ENTER
                        ...
                        try:
                            ...
                            self._log(f"[Bridge] PRESS ENTER failed: {e}")
                            success = False
                    elif kind == "BACK":
                        back_url = value
                        self._log(f"[Bridge] Received BACK, navigating to: {back_url}")
                        action_repr = "<navigation: BACK>"
                        self.driver.get(back_url)
                        success = True  # BACK operation treated as success
                    else:
                        self._log(f"[Bridge] Unimplemented action: {kind}")
                        # Log error but continue executing subsequent commands
                        # Reason: could be an unsupported command from LLM, shouldn't block subsequent commands
                        self._log(f"[Bridge] Skipping unimplemented action, continuing with subsequent commands")
                        continue  # Skip this iteration, continue next command
                    if not success:
                        self._log(f"[Bridge] Execution failed: {raw_cmd}")
                        # DEBUG: check wrapper errors after failure
                        self._check_wrapper_errors()
                        ...
                        try:
                            self.driver.save_screenshot(screenshot_path)
                            self._log(f"[Bridge] Failure screenshot: {screenshot_path}")
                        except Exception:
                            pass
                        self._log(f"[Bridge] Skipping failed operation, continuing with subsequent commands")
                    ...
                    try:
                        if kind in ('SUBMIT', 'TRIGGER'):
                            ...
                            try:
                                ...
                                self._log(f"[Bridge] URL change detected, redirect complete")
                            except TimeoutException:
                                # Timeout without URL change is also OK (could be AJAX submit or in-page operation)
                                self._log(f"[Bridge] URL unchanged after 3 second wait, continuing execution")
                        else:
                            time.sleep(0.5)
                    except Exception:
                        pass
                    ...
                    if page_changed:
                        self._log(f"[Bridge] Page navigation detected: {old_url} -> {new_url}")
                    ...
                    screenshot_path = os.path.join(screenshot_dir, screenshot_filename)
                    self.driver.save_screenshot(screenshot_path)
                    self._log(f"[Bridge] Screenshot: {screenshot_path}")
                    ...
                    if page_changed and kind in ["SUBMIT", "CLICK", "BACK"]:
                        self._log(f"[Bridge] {kind} action caused page navigation, stopping remaining commands this round")
                        self._log(f"[Bridge] New page will be processed in the next loop iteration")
                ...
                if saw_continue:
                    ...
                    if continue_loops >= MAX_CONTINUE_LOOPS:
                        self._log(f"[Bridge] CONTINUE count reached limit {MAX_CONTINUE_LOOPS}, ending task")
                        self._log(f"[Bridge] Hint: task may be too complex, or LLM is overusing CONTINUE")
                        task.set_completed()
                        break
                    else:
                        self._log(f"[Bridge] CONTINUE detected, continuing next round ({continue_loops}/{MAX_CONTINUE_LOOPS})")
                        self._log(f"[Bridge] LLM believes task needs to continue, will rescan page and ask again")
...
# Below is adapted for the one-step version. Above is the multi-step version.
# --- Main loop: callback tracer after each step, recording jump info ---
    # def run_task(self, task, photo_dir,logging_in=False):
    #     # Open task log file
    #     self._open_task_log(photo_dir, task.task_id)
    ...
    #         # Which one to choose?
    #         # self.sensors.update_logic_page()
    #         # self.sensors.update_abstract_page()
    #         page_description = self.sensors.get_abstract_page()
    #         self._log(f"[Bridge] LLM input\n{page_description}")
    #         prompt = self.generate_bridge_onestep_prompt(task, page_description, task.get_history())
    ...
    #                 # Update last_url to trigger new page loading logic
    #                 last_url = new_url
    #                 break  # Break out of commands loop, back to while loop
